...
import { copyForProduct } from "@/content/descriptions";
...
// Descrierile scrise de noi (content/descriptions.js) au prioritate față de
// textele din WooCommerce, ca munca de redactare să reziste la orice reimport.
function withCopy(product) {
  if (!product) return product;
  const copy = copyForProduct(product);
  if (!copy) return product;
  return {
    ...product,
    shortDescription: copy.shortDescription || product.shortDescription,
...
export async function getProducts({ limit, category, search, sort } = {}) {
  let list = (productsLocal.length ? [...productsLocal] : await wpProducts({ perPage: 100 })).map(withCopy);
...
export async function getProduct(slug) {
  ...
  return withCopy(local || (await wpProduct(slug)));
}
