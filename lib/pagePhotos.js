...
/**
 * Conținutul unei pagini fără poze și fără primele paragrafe.
 *
 * Pozele le arătăm separat, în mozaic; primele paragrafe stau lângă
 * fotografia de sus. Restul trebuie să curgă ca articol, cu titlurile lui
 * cu tot — nu strivit într-o coloană îngustă.
 */
export function contentAfterIntro(html, sarite = 2) {
  if (!html) return "";
  let rest = html
    .replace(/<figure[^>]*>[\s\S]*?<\/figure>/gi, "")
    .replace(/<img[^>]*>/gi, "");
  for (let i = 0; i < sarite; i++) {
    const m = rest.match(/<p[^>]*>[\s\S]*?<\/p>/i);
    if (!m) break;
    if (stripHtml(m[0]).length < 40) { i--; }
...
export async function aboutPage(slug = "despre") {
  ...
  return {
    ...
    rest: contentAfterIntro(page.content),
    raw: page.content || "",
  };
}
